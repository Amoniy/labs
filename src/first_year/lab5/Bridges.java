...
public class Bridges {
        ...
        for (int i = 0; i < map[v].size(); i++) {
            ...
            if (nextVertice == parentVertice) {
                ...
                if (!used[nextVertice]) {//downVertics
                    ...
                    if ((arrayForChecking[nextVertice] > timeIn[v])) {
                        answer.add(answer.size(), map[v].get(i)[1]);
                    }
                } else {//upVertice
                    arrayForChecking[v] = Math.min(arrayForChecking[v], arrayForChecking[nextVertice]);//timeIn
                }
            }
        }
    }
    ...
    public static void main(String[] args) throws Exception {
        String sourceFileName = "Bridges.in";
        String destinationFileName = "Bridges.out";
        java.io.FileInputStream fileInputStream = new java.io.FileInputStream(sourceFileName);
        java.io.FileOutputStream fileOutputStream = new java.io.FileOutputStream(destinationFileName);
        BufferedReader br = new BufferedReader(new FileReader(sourceFileName));
        String split = "[ ]+";
        FileWriter writer = new FileWriter("Bridges.out", true);
